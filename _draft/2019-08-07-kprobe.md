...
## 优化之后的kretprobe
1.为kprobe关联一个optimized_kprobe对象，它有一个detour bufferer,保存有一段指令,之后紧接着将被优化的指令拷贝，最后通过jmp跳转回被优化区域的下一条指令地址.
这段指令集原来是保存在代码段中，是一段指令范式，在优化期间直接复制这段指令，里面有些占位指令用来放置动态的地址的，将地址动态的填充到固定的偏移上，这样就组装成了一段指令集，之后只需要为它设置可执行属性，刷新icache,这段buffer就完成了数据到指令的变身。

这段指令要完成的工作也非常简单:
2.准备工作:
在栈上填充一份pt_regs,填充寄存器:rdi,rsi分别指向optimized_kprobe对象和pt_regs
3.实际工作:
调用optimized_callback这样就能通过optimized_callback调用到kretprobe的pre_handler_kretprobe，设置上一帧的rip为:kretprobe_trampoline
4.恢复现场:
把栈上保存的寄存器数据恢复到寄存器中,之后jmp到被优化区域的下一条指令地址
5.函数执行完毕后，这部分就和优化前的处理是相同的,通过kretprobe_trampoline先将正确的返回地址设置到rip中，然后调用用户注册的handler,之后ret到正确的caller
## kprobes的注意事项
